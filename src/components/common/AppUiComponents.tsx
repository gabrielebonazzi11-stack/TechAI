...
export function QuickCalcCard({ result, theme, isDark }: { result: QuickCalcResult; theme: Theme; isDark: boolean }) {
  ...
  // ── Grafici ─────────────────────────────────────────────────────────────
  const isGoodman = /fatica|goodman/i.test(result.title + result.scheme);

  const parseVal = (rows: string[], patterns: RegExp[]): number | null => {
    for (const row of rows) {
      if (patterns.some(p => p.test(row))) {
        const m = row.match(/-?\d+(?:[.,]\d+)?/g);
        if (m) {
          const v = Number(m[m.length - 1].replace(",", "."));
  ...
  const allRows = [...(result.values || []), ...(result.sectionValues || [])];
  const Re   = parseVal(allRows, [/Re\/Rp|Re usato|Rp0/i]) ?? 0;
  const Rm   = parseVal(allRows, [/Rm usato/i]) ?? 0;
  const Sn   = parseVal(allRows, [/Sn usato/i]) ?? 0;
  const sigM = parseVal(allRows, [/σm\s*=/i, /sigma_?m\s*=/i]) ?? 0;
  const sigA = parseVal(allRows, [/σa\s*=/i, /sigma_?a\s*=/i]) ?? 0;
  const sigEq = result.equivalentStress;
  const nReq  = requiredSafety;

  // Colori
  const green = "#22c55e", red = "#ef4444", amber = "#f59e0b";
  const chartBg   = isDark ? "#0b0b0b" : "#f8fafc";
  const axisColor = isDark ? "#334155" : "#cbd5e1";
  const textColor = isDark ? "#94a3b8" : "#64748b";

  // ── Von Mises gauge ──────────────────────────────────────────────────────
  const VonMisesGauge = () => {
    if (!Re || Re <= 0 || sigEq <= 0) return null;
    const W = 340, H = 90;
    const pad = 20;
    const barW = W - pad * 2;
    const limit = Math.max(Re, sigEq) * 1.15;
    const xEq  = pad + (sigEq / limit) * barW;
    const xRe  = pad + (Re   / limit) * barW;
    const xReq = pad + (Re   / nReq   / limit) * barW;
    const fillColor = sigEq <= Re / nReq ? green : sigEq <= Re ? amber : red;

    return (
      <div style={{ background: chartBg, border: `1px solid ${axisColor}`, borderRadius: 12, padding: "14px 16px", marginTop: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: theme.primary, marginBottom: 8 }}>DIAGRAMMA VON MISES</div>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
          {/* sfondo barra */}
          <rect x={pad} y={32} width={barW} height={20} rx={4} fill={isDark ? "#1e293b" : "#e2e8f0"} />
          {/* barra tensione */}
          <rect x={pad} y={32} width={Math.min(xEq - pad, barW)} height={20} rx={4} fill={fillColor} opacity={0.85} />
          {/* linea Re */}
          <line x1={xRe} y1={24} x2={xRe} y2={60} stroke={green} strokeWidth={2} strokeDasharray="4 2" />
          <text x={xRe} y={18} textAnchor="middle" fontSize={10} fill={green} fontWeight={700}>Re={Re.toFixed(0)}</text>
          {/* linea n_req */}
          {xReq > pad && xReq < xRe - 10 && (
            <>
              <line x1={xReq} y1={24} x2={xReq} y2={60} stroke={amber} strokeWidth={1.5} strokeDasharray="3 2" />
              <text x={xReq} y={18} textAnchor="middle" fontSize={9} fill={amber}>Re/n</text>
            </>
          )}
          {/* etichetta sigma_eq */}
          <text x={Math.min(xEq, W - pad - 10)} y={80} textAnchor="middle" fontSize={10} fill={fillColor} fontWeight={700}>
            σeq={sigEq.toFixed(1)} MPa
          </text>
          {/* assi label */}
          <text x={pad} y={H - 2} fontSize={9} fill={textColor}>0</text>
          <text x={W - pad} y={H - 2} textAnchor="end" fontSize={9} fill={textColor}>{limit.toFixed(0)} MPa</text>
        </svg>
        <div style={{ display: "flex", gap: 12, marginTop: 4, flexWrap: "wrap" }}>
          {[
            { color: green, label: `Re = ${Re.toFixed(0)} MPa (limite materiale)` },
            { color: amber, label: `Re/n = ${(Re/nReq).toFixed(0)} MPa (limite con n=${nReq.toFixed(1)})` },
            { color: fillColor, label: `σeq = ${sigEq.toFixed(2)} MPa (calcolata)` },
          ].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: textColor }}>
              <span style={{ width: 12, height: 12, borderRadius: 2, background: item.color, display: "inline-block", flexShrink: 0 }} />
              {item.label}
            </div>
          ))}
        </div>
  ...
  const GoodmanDiagram = () => {
    if (!isGoodman || !Rm || !Sn || Rm <= 0 || Sn <= 0) return null;
    const W = 340, H = 240;
    const pl = 46, pr = 16, pt = 16, pb = 36;
    const chartW = W - pl - pr;
    const chartH = H - pt - pb;
    const xMax = Rm * 1.1, yMax = Sn * 1.25;

    const toX = (v: number) => pl + (v / xMax) * chartW;
    const toY = (v: number) => pt + chartH - (v / yMax) * chartH;

    // Goodman line: (0, Sn) → (Rm, 0)
    const gx1 = toX(0), gy1 = toY(Sn);
    const gx2 = toX(Rm), gy2 = toY(0);

    // Punto di lavoro
    const px = toX(sigM), py = toY(sigA);

    // Margine: raggiungimento della linea di Goodman partendo dall'origine
    // Intersezione retta OP con Goodman: parametrico
    // P(t) = (t*sigM, t*sigA); Goodman: σa/Sn + σm/Rm = 1
    // t*(sigA/Sn + sigM/Rm) = 1 → t = 1/(sigA/Sn + sigM/Rm)
    const denom = (sigA / Sn) + (Math.max(sigM, 0) / Rm);
    const tInt  = denom > 0 ? 1 / denom : 999;
    const intX  = toX(sigM * tInt);
    const intY  = toY(sigA * tInt);

    const ptOk = sigM >= 0 && denom > 0 && (1 / denom) >= 1;
    ...
    return (
      <div style={{ background: chartBg, border: `1px solid ${axisColor}`, borderRadius: 12, padding: "14px 16px", marginTop: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: theme.primary, marginBottom: 8 }}>DIAGRAMMA DI GOODMAN</div>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
          {/* Grid */}
          {Array.from({ length: ticks + 1 }).map((_, i) => {
            const xv = (xMax / ticks) * i;
            const yv = (yMax / ticks) * i;
            return (
              <g key={i}>
                <line x1={toX(xv)} y1={pt} x2={toX(xv)} y2={pt + chartH} stroke={axisColor} strokeWidth={0.5} />
                <line x1={pl} y1={toY(yv)} x2={pl + chartW} y2={toY(yv)} stroke={axisColor} strokeWidth={0.5} />
                <text x={toX(xv)} y={pt + chartH + 12} textAnchor="middle" fontSize={9} fill={textColor}>{xv.toFixed(0)}</text>
                {i > 0 && <text x={pl - 4} y={toY(yv) + 3} textAnchor="end" fontSize={9} fill={textColor}>{yv.toFixed(0)}</text>}
              </g>
            );
          })}
          {/* Assi */}
          <line x1={pl} y1={pt} x2={pl} y2={pt + chartH} stroke={textColor} strokeWidth={1} />
          <line x1={pl} y1={pt + chartH} x2={pl + chartW} y2={pt + chartH} stroke={textColor} strokeWidth={1} />
          {/* Label assi */}
          <text x={pl + chartW / 2} y={H - 2} textAnchor="middle" fontSize={10} fill={textColor}>σm (MPa)</text>
          <text x={10} y={pt + chartH / 2} textAnchor="middle" fontSize={10} fill={textColor} transform={`rotate(-90, 10, ${pt + chartH / 2})`}>σa (MPa)</text>
          {/* Punti chiave assi */}
          <text x={toX(Rm)} y={pt + chartH + 12} textAnchor="middle" fontSize={9} fill={green} fontWeight={700}>Rm={Rm.toFixed(0)}</text>
          <text x={pl - 4} y={toY(Sn) + 3} textAnchor="end" fontSize={9} fill={green} fontWeight={700}>Sn={Sn.toFixed(0)}</text>
          {/* Linea di Goodman */}
          <line x1={gx1} y1={gy1} x2={gx2} y2={gy2} stroke={green} strokeWidth={2} />
          {/* Area sicura */}
          <polygon
            points={`${pl},${pt + chartH} ${gx1},${gy1} ${gx2},${gy2}`}
            fill={green} fillOpacity={0.08}
          />
          {/* Retta dall'origine al punto (proiezione margine) */}
          {sigM >= 0 && sigA > 0 && (
            <line x1={toX(0)} y1={toY(0)} x2={intX} y2={intY} stroke={ptOk ? green : red} strokeWidth={1} strokeDasharray="4 2" opacity={0.6} />
          )}
          {/* Punto di lavoro */}
          <circle cx={px} cy={py} r={6} fill={ptOk ? green : red} />
          <circle cx={px} cy={py} r={10} fill="none" stroke={ptOk ? green : red} strokeWidth={1.5} opacity={0.4} />
          <text x={px + 12} y={py - 4} fontSize={10} fill={ptOk ? green : red} fontWeight={700}>
            ({sigM.toFixed(0)}, {sigA.toFixed(0)})
          </text>
          {/* Intersezione */}
          {sigA > 0 && sigM >= 0 && (
            <circle cx={intX} cy={intY} r={4} fill="none" stroke={textColor} strokeWidth={1.5} />
          )}
        </svg>
        <div style={{ display: "flex", gap: 12, marginTop: 4, flexWrap: "wrap" }}>
          {[
            { color: green, label: `Linea Goodman (Rm=${Rm.toFixed(0)}, Sn=${Sn.toFixed(0)})` },
            { color: ptOk ? green : red, label: `Punto di lavoro σm=${sigM.toFixed(1)}, σa=${sigA.toFixed(1)} MPa` },
          ].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: textColor }}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  textarea: { flex: 1, minWidth: 0, background: "none", border: "none", outline: "none", textAlign: "center", fontSize: 16, resize: "none", padding: "10px 0", overflow: "hidden" },
